...
[Hostinger](https://hostinger.com) is a popular shared hosting provider. This guide covers deploying PDFusion to Hostinger's shared hosting.
...
RewriteCond %{HTTP_HOST} ^PDFusion\.com [NC]
RewriteRule ^(.*)$ https://www.PDFusion.com/$1 [L,R=301]
...
Make sure the `.html` extension rule and language routes are correctly configured. PDFusion uses static HTML files, not SPA routing:
