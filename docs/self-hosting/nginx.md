...
Host PDFusion on your own server using Nginx.
...
Create `/etc/nginx/sites-available/PDFusion`:
...
```bash
sudo ln -s /etc/nginx/sites-available/PDFusion /etc/nginx/sites-enabled/
sudo nginx -t
