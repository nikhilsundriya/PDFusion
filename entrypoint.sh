...
echo "Starting PDFusion with PUID=$PUID PGID=$PGID"

addgroup -g "$PGID" PDFusion 2>/dev/null || true
adduser -u "$PUID" -G PDFusion -D -H -s /sbin/nologin PDFusion 2>/dev/null || true
