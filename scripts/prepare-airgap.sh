...
# ============================================================
# PDFusion Air-Gapped Deployment Preparation Script
# ============================================================
...
# contains everything needed to deploy PDFusion offline.
#
# Usage:
...
WASM_BASE_URL=""
IMAGE_NAME="PDFusion"
OUTPUT_DIR="./PDFusion-airgap-bundle"
SIMPLE_MODE=""
BASE_URL=""
COMPRESSION_MODE=""
LANGUAGE=""
...
usage() {
  cat <<'EOF'
PDFusion Air-Gapped Deployment Preparation
...
OPTIONS:
  --image-name <name>     Docker image name          (default: PDFusion)
  --output-dir <path>     Output bundle directory     (default: ./PDFusion-airgap-bundle)
...
if [ ! -f "package.json" ] || [ ! -f "src/js/const/cdn-version.ts" ]; then
  error "This script must be run from the PDFusion project root."
...
interactive_mode() {
  echo ""
  echo -e "${BOLD}============================================================${NC}"
  echo -e "${BOLD}  PDFusion Air-Gapped Deployment Preparation${NC}"
  ...
  echo "      https://cdn.mycompany.local/PDFusion"
  echo ""
  while true; do
  ...
  echo "    Replace the default PDFusion name, logo, and footer text."
  read -r -p "    Brand name [PDFusion]: " input
  BRAND_NAME="${input:-}"
  if [ -n "$BRAND_NAME" ]; then
    ...
    BRAND_LOGO="${input:-}"
    read -r -p "    Footer text [© 2026 PDFusion. All rights reserved.]: " input
    FOOTER_TEXT="${input:-}"
  fi
  echo ""
  ...
  echo "  Brand Name:     ${BRAND_NAME:-PDFusion (default)}"
...
echo ""
echo -e "${BOLD}============================================================${NC}"
echo -e "${BOLD}  PDFusion Air-Gapped Bundle Preparation${NC}"
...
if ls "$OUTPUT_DIR"/*.tgz "$OUTPUT_DIR"/PDFusion.tar "$OUTPUT_DIR"/setup.sh 2>/dev/null | head -1 &>/dev/null; then
...
if [ "$SKIP_WASM" = true ]; then
  ...
  wasm_missing=false
  if ! ls "$OUTPUT_DIR"/PDFusion-pymupdf-wasm-*.tgz &>/dev/null; then
    error "Missing: PDFusion-pymupdf-wasm-*.tgz"
    wasm_missing=true
  fi
  if ! ls "$OUTPUT_DIR"/PDFusion-gs-wasm-*.tgz &>/dev/null; then
    error "Missing: PDFusion-gs-wasm-*.tgz"
    wasm_missing=true
  fi
  ...
  info "Downloading @PDFusion/pymupdf-wasm@${PYMUPDF_VERSION}..."
  if ! (cd "$WASM_TMP" && npm pack "@PDFusion/pymupdf-wasm@${PYMUPDF_VERSION}" --quiet 2>&1); then
    error "Failed to download @PDFusion/pymupdf-wasm@${PYMUPDF_VERSION}"
    error "Check your internet connection and that the package exists on npm."
    exit 1
  fi

  info "Downloading @PDFusion/gs-wasm@${GS_VERSION}..."
  if ! (cd "$WASM_TMP" && npm pack "@PDFusion/gs-wasm@${GS_VERSION}" --quiet 2>&1); then
    error "Failed to download @PDFusion/gs-wasm@${GS_VERSION}"
    error "Check your internet connection and that the package exists on npm."
    exit 1
  fi
  ...
  info "  PyMuPDF:      $(filesize "$OUTPUT_DIR"/PDFusion-pymupdf-wasm-*.tgz)"
  info "  Ghostscript:  $(filesize "$OUTPUT_DIR"/PDFusion-gs-wasm-*.tgz)"
...
if [ "$SKIP_DOCKER" = true ]; then
  ...
  if [ -f "$OUTPUT_DIR/PDFusion.tar" ]; then
    success "Reusing existing PDFusion.tar"
  elif docker image inspect "$IMAGE_NAME" &>/dev/null; then
    step "Exporting existing Docker image"
    docker save "$IMAGE_NAME" -o "$OUTPUT_DIR/PDFusion.tar"
    success "Exported: $(filesize "$OUTPUT_DIR/PDFusion.tar")"
  else
    warn "No Docker image '${IMAGE_NAME}' found and no PDFusion.tar in output."
...
# ============================================================
# PDFusion Air-Gapped Setup Script
# Generated on $(date -u +"%Y-%m-%d %H:%M:%S UTC")
# PDFusion v${APP_VERSION}
# ============================================================
...
echo ""
echo "============================================================"
echo "  PDFusion Air-Gapped Setup"
...
if [ -f "\${SCRIPT_DIR}/PDFusion.tar" ]; then
  docker load -i "\${SCRIPT_DIR}/PDFusion.tar"
  echo "  Docker image '\${IMAGE_NAME}' loaded."
else
  echo "  WARNING: PDFusion.tar not found. Skipping Docker load."
...
tar xzf "\${SCRIPT_DIR}"/PDFusion-pymupdf-wasm-*.tgz -C "\${WASM_DIR}/pymupdf" --strip-components=1

# Ghostscript: browser expects gs.js and gs.wasm at root
echo "  Extracting Ghostscript..."
TEMP_GS="\$(mktemp -d)"
tar xzf "\${SCRIPT_DIR}"/PDFusion-gs-wasm-*.tgz -C "\${TEMP_GS}"
...
# --- Step 3: Start PDFusion ---
echo ""
echo "[3/3] Ready to start PDFusion"
echo ""
echo "  To start manually:"
echo "    docker run -d --name PDFusion -p \${DOCKER_PORT}:8080 --restart unless-stopped \${IMAGE_NAME}"
echo ""
echo "  Then open: http://localhost:\${DOCKER_PORT}"
echo ""

read -r -p "Start PDFusion now? (y/N): " REPLY
if [[ "\${REPLY:-}" =~ ^[Yy]$ ]]; then
  docker run -d --name PDFusion -p "\${DOCKER_PORT}:8080" --restart unless-stopped "\${IMAGE_NAME}"
  echo ""
  echo "  PDFusion is running at http://localhost:\${DOCKER_PORT}"
fi
SETUP_EOF
...
# PDFusion Air-Gapped Deployment Bundle

**PDFusion v${APP_VERSION}** | Generated on $(date -u +"%Y-%m-%d %H:%M:%S UTC")
...
| \`PDFusion-pymupdf-wasm-${PYMUPDF_VERSION}.tgz\` | PyMuPDF WASM module |
| \`PDFusion-gs-wasm-${GS_VERSION}.tgz\` | Ghostscript WASM module |
...
3. Optionally start the PDFusion container
...
# PyMuPDF
tar xzf PDFusion-pymupdf-wasm-${PYMUPDF_VERSION}.tgz -C ./wasm/pymupdf --strip-components=1

# Ghostscript (extract assets/ to root)
TEMP_GS=\$(mktemp -d)
tar xzf PDFusion-gs-wasm-${GS_VERSION}.tgz -C \$TEMP_GS
...
\`\`\`bash
docker run -d --name PDFusion -p 3000:8080 --restart unless-stopped ${IMAGE_NAME}
\`\`\`
