...
PDFusion uses a Cloudflare Worker to proxy WASM library requests, bypassing CORS restrictions when loading AGPL-licensed components (PyMuPDF, Ghostscript, CoherentPDF) from external sources.
...
# Go to Workers & Pages > PDFusion-wasm-proxy > Settings > Variables
```
...
```bash
VITE_WASM_PYMUPDF_URL=https://PDFusion-wasm-proxy.<your-subdomain>.workers.dev/pymupdf/
VITE_WASM_GS_URL=https://PDFusion-wasm-proxy.<your-subdomain>.workers.dev/gs/
VITE_WASM_CPDF_URL=https://PDFusion-wasm-proxy.<your-subdomain>.workers.dev/cpdf/
```

**Option B: Manual per-user configuration**

In PDFusion's Advanced Settings (wasm-settings.html), enter:

| Module      | URL                                                                 |
| ----------- | ------------------------------------------------------------------- |
| PyMuPDF     | `https://PDFusion-wasm-proxy.<your-subdomain>.workers.dev/pymupdf/` |
| Ghostscript | `https://PDFusion-wasm-proxy.<your-subdomain>.workers.dev/gs/`      |
| CoherentPDF | `https://PDFusion-wasm-proxy.<your-subdomain>.workers.dev/cpdf/`    |

## Custom Domain (Optional)

To use a custom domain like `wasm.PDFusion.com`:

1. Add route in `wasm-wrangler.toml`:

```toml
routes = [
  { pattern = "wasm.PDFusion.com/*", zone_name = "PDFusion.com" }
]
```
