...
**PDFusion** is a powerful, privacy-first, client-side PDF toolkit that is self hostable and allows you to manipulate, edit, merge, and process PDF files directly in your browser. No server-side processing is required, ensuring your files remain secure and private.

[![Docker Downloads](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fghcr-badge.elias.eu.org%2Fapi%2Fnikhilsundriya%2FPDFusion%2FPDFusion&query=%24.downloadCount&logo=docker&label=Docker%20Downloads&color=blue)](https://github.com/nikhilsundriya/pkgs/container/PDFusion) [![Ko-fi](https://img.shields.io/badge/Buy%20me%20a%20Coffee-yellow?logo=kofi&style=flat-square)](https://ko-fi.com/nikhilsundriya) ![GitHub Stars](https://img.shields.io/github/stars/nikhilsundriya/PDFusion?style=social)
[![Sponsor me on GitHub](https://img.shields.io/badge/Sponsor-%E2%9D%A4-ff69b4)](https://github.com/sponsors/nikhilsundriya)

![PDFusion Tools](public/images/PDFusion-tools.png)
...
[![Documentation](https://img.shields.io/badge/Docs-VitePress-646cff?style=for-the-badge&logo=vite&logoColor=white)](https://PDFusion.com/docs/)

Visit our [Documentation](https://PDFusion.com/docs/) for:
...
PDFusion is **dual-licensed** to fit your needs:
...
📖 For more details, see our [Licensing Page](https://PDFusion.com/licensing.html)

### AGPL Components (Pre-configured via CDN)

PDFusion does **not** bundle AGPL-licensed processing libraries in its source code, but **pre-configures CDN URLs** so all features work out of the box with zero setup:
...
[![Star History Chart](https://api.star-history.com/svg?repos=nikhilsundriya/PDFusion&type=Date)](https://star-history.com/#nikhilsundriya/PDFusion&Date)

---

## 💖 Thank You to Our Sponsors

We're incredibly grateful to all our sponsors and supporters who help keep PDFusion free and open source!

[![Sponsor me on GitHub](https://img.shields.io/badge/Become%20a%20Sponsor-%E2%9D%A4-ff69b4?style=for-the-badge)](https://github.com/sponsors/nikhilsundriya)
[![Buy me a Coffee](https://img.shields.io/badge/Buy%20me%20a%20Coffee-yellow?style=for-the-badge&logo=kofi)](https://ko-fi.com/nikhilsundriya)
...
- **High Performance**: Built with modern web technologies, PDFusion is fast and efficient, handling even large PDF files with ease.
- **Completely Free**: PDFusion is a free and open-source tool for everyone.

---

## 🛠️ Features / Tools Supported

PDFusion offers a comprehensive suite of tools to handle all your PDF needs.
...
PDFusion is available in multiple languages:
...
Want to help translate PDFusion into your language? Check out our [Translation Guide](TRANSLATION.md)!

---

## 🚀 Getting Started

You can run PDFusion locally for development or personal use.
...
Run PDFusion instantly from GitHub Container Registry (Recommended):
...
```bash
docker run -p 3000:8080 PDFusionteam/PDFusion:latest
```

**Podman (GHCR):**

```bash
podman run -p 3000:8080 ghcr.io/nikhilsundriya/PDFusion:latest
```

**Podman (Docker Hub):**

```bash
podman run -p 3000:8080 docker.io/PDFusionteam/PDFusion:latest
```
...
It is very straightforward to host your own instance of PDFusion using a static web page hosting service. Plus, services such as Netlify, Vercel, and GitHub Pages all offer a free tier for getting started. See [Static Hosting](https://github.com/nikhilsundriya/blob/main/STATIC-HOSTING.md) for details.

### 🏠 Self-Hosting Locally

Since PDFusion is fully client-side, all processing happens in the user's browser and no server-side processing is required. This means you can host PDFusion as simple static files on any web server or hosting platform.

**Download from Releases (Recommended):**

The easiest way to self-host is to download the pre-built distribution file from our [GitHub releases](https://github.com/nikhilsundriya/releases). Each release includes a `dist-{version}.zip` file that contains all necessary files for self-hosting.

1. Go to [PDFusion Releases](https://github.com/nikhilsundriya/releases)
...
PDFusion supports different compression modes for optimized builds:

```bash
# Gzip only (smallest Docker image size)
npm run build:gzip
docker build --build-arg COMPRESSION_MODE=g -t PDFusion:gzip .

# Brotli only (best compression ratio)
npm run build:brotli
docker build --build-arg COMPRESSION_MODE=b -t PDFusion:brotli .

# No compression (fastest build time)
npm run build:original
docker build --build-arg COMPRESSION_MODE=o -t PDFusion:original .

# All formats (default, maximum browser compatibility)
npm run build:all
docker build --build-arg COMPRESSION_MODE=all -t PDFusion:all .
```
...
PDFusion can use jsDelivr CDN to serve large WASM files (LibreOffice, Ghostscript, PyMuPDF) for improved performance and reduced bandwidth costs:
...
```
PDFusion-airgap-bundle/
  PDFusion.tar              # Docker image
...
| `--image-name <name>`   | Docker image tag                                 | `PDFusion`                        |
| `--output-dir <path>`   | Output bundle directory                          | `./PDFusion-airgap-bundle`        |
...
> [!IMPORTANT]
> WASM files must be served from the **same origin** as the PDFusion app. Web Workers use `importScripts()` which cannot load scripts cross-origin. For example, if PDFusion runs at `https://internal.example.com`, the WASM base URL should also be `https://internal.example.com/wasm`.
...
- `PDFusion.tar` — the Docker image
- `PDFusion-pymupdf-wasm-0.11.14.tgz` — PyMuPDF WASM package
- `PDFusion-gs-wasm-*.tgz` — Ghostscript WASM package
...
tar xzf PDFusion-pymupdf-wasm-0.11.14.tgz -C ./wasm/pymupdf --strip-components=1
tar xzf PDFusion-gs-wasm-*.tgz -C ./wasm/gs --strip-components=1
...
PDFusion can also be hosted from a subdirectory (e.g., `example.com/tools/PDFusion/`):

```bash

# Example:
# 1. Build the app with the specific BASE_URL. BASE_URL must have a trailing and leading slash. The BASE_URL can be any url of your choice. Here we are using /tools/PDFusion/ as an example.

BASE_URL=/tools/PDFusion/ npm run build
...
mkdir -p serve-test/tools/PDFusion

# 3. Copy all files from the 'dist' folder into that nested directory
cp -r dist/* serve-test/tools/PDFusion/

# 4. Serve the 'serve-test' folder
npx serve serve-test
```

The website can be accessible at: `http://localhost:3000/tools/PDFusion/`
...
PDFusion's Docker image also supports the `BASE_URL` build argument for subdirectory deployments:

```bash
# Build for subdirectory deployment
docker build --build-arg BASE_URL=/PDFusion/ -t PDFusion .

# Run the container
docker run -p 3000:8080 PDFusion

# The app will be accessible at http://localhost:3000/PDFusion/
```
...
> [!IMPORTANT]
>
> - Always include trailing slashes in `BASE_URL` (e.g., `/PDFusion/` not `/PDFusion`)
...
services:
  PDFusion:
    image: ghcr.io/nikhilsundriya/PDFusion:latest # Recommended
    # image: PDFusionteam/PDFusion:latest     # Alternative: Docker Hub
...
For Linux production deployments, you can run PDFusion as a systemd service using [Podman Quadlet](https://docs.podman.io/en/latest/markdown/podman-systemd.unit.5.html).

Create `~/.config/containers/systemd/PDFusion.container`:
...
systemctl --user enable --now PDFusion
```

For detailed Quadlet configuration, see [Self-Hosting Docker Guide](https://PDFusion.com/docs/self-hosting/docker).

### 🏢 Simple Mode for Internal Use

For organizations that want a clean, distraction-free interface focused solely on PDF tools, PDFusion supports a **Simple Mode** that hides all branding and marketing content.
...
Replace the default PDFusion logo, name, and footer text with your own. Branding is configured via environment variables at **build time** and works across all deployment methods (Docker, static hosting, air-gapped VMs).

| Variable           | Description                             | Default                                 |
| ------------------ | --------------------------------------- | --------------------------------------- |
| `VITE_BRAND_NAME`  | Brand name shown in header and footer   | `PDFusion`                              |
...
PDFusion runs as a non-root user using nginx-unprivileged for enhanced security:
...
```bash
docker build -t PDFusion .
docker run -p 8080:8080 PDFusion
```
...
docker run -d -p 3000:8080 -e PUID=1000 -e PGID=1000 PDFusion-nonroot
```
...
3. **Update allowed origins** — open `cors-proxy-worker.js` and change `ALLOWED_ORIGINS` to your domain:
   ...
   > [!IMPORTANT]
   > Without this step, the proxy will reject all requests from your site with a 403 error. The default only allows `PDFusion.com`.
...
5. **Note your worker URL** (e.g., `https://PDFusion-cors-proxy.your-subdomain.workers.dev`)
...
6. **Set the environment variable when building:**
   ...
   Or with Docker:
   ```bash
   docker build --build-arg VITE_CORS_PROXY_URL="https://your-worker-url.workers.dev" -t your-PDFusion .
   ```
...
PDFusion uses client-side HMAC as a deterrent against casual abuse, but accepts this tradeoff due to its fully client-side architecture. To enable:
...
PDFusion supports semantic versioning with multiple container tags available:

**GitHub Container Registry (Recommended):**

- **Latest**: `ghcr.io/nikhilsundriya/PDFusion:latest`
- **Specific Version**: `ghcr.io/nikhilsundriya/PDFusion:1.0.0`
- **Version with Prefix**: `ghcr.io/nikhilsundriya/PDFusion:v1.0.0`

**Docker Hub:**

- **Latest**: `PDFusionteam/PDFusion:latest`
- **Specific Version**: `PDFusionteam/PDFusion:1.0.0`
- **Version with Prefix**: `PDFusionteam/PDFusion:v1.0.0`
...
PDFusion was originally built using **HTML**, **CSS**, and **vanilla JavaScript**. As the project grew, it was migrated to a modern stack for better maintainability and scalability:
...
Have an idea for a new tool or an improvement? [Open an issue](https://github.com/nikhilsundriya/issues) to discuss it first.
...
PDFusion wouldn't be possible without the amazing open-source tools and libraries that power it. We'd like to extend our heartfelt thanks to the creators and maintainers of:
...
> [!NOTE]
> AGPL-licensed libraries are not bundled in PDFusion's source code. They are loaded at runtime from CDN (pre-configured) and can be overridden via environment variables or Advanced Settings.
