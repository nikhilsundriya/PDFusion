...
```bash
helm install PDFusion ./chart
kubectl port-forward deploy/PDFusion 8080:8080
...
- **`image.repository`**: container image repo (default: `ghcr.io/nikhilsundriya/PDFusion-simple`)
- **`image.tag`**: image tag (default: `Chart.appVersion`)
- **`image.pullPolicy`**: default `IfNotPresent`

### Ports

- **`containerPort`**: container listen port (**8080** for the PDFusion nginx image)
...
# produces PDFusion-<version>.tgz
helm push PDFusion-*.tgz oci://ghcr.io/$GHCR_USERNAME/charts
```
...
```bash
helm upgrade --install PDFusion oci://ghcr.io/$GHCR_USERNAME/charts/PDFusion --version 1.0.0
```
